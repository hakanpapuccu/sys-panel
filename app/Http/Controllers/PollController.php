        ...
        foreach ($poll->questions as $question) {
            if (in_array($question->type, ['radio', 'checkbox'])) {
                ...
                foreach ($question->options as $option) {
                    $count = $question->answers()->where('poll_option_id', $option->id)->count();
                    $optionCounts[$option->option_text] = $count;
                }
                $stats[$question->id] = $optionCounts;
            } else {
                // For text answers, we might just list the latest ones or count them
                $stats[$question->id] = $question->answers()->count();
            }
        }

        return view('admin.polls.show', compact('poll', 'stats'));
    }
