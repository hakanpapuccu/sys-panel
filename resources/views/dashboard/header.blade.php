...
<body>
		@php($authUser = $currentUser ?? auth()->user())
	       ...
	       <div class="nav-header">
	            <a href="{{ route('dashboard') }}" class="brand-logo">
					...
					<div class="brand-title">
						...
						<span class="brand-sub-title">{{ $authUser->is_admin ? 'Yönetici' : 'Personel' }}</span>
				</div>
            </a>
								...
								<li class="nav-item dropdown notification_dropdown">
	                                <button class="nav-link btn btn-link border-0 p-0" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Bildirimler">
										<svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
		                                    ...
		                                    @if(($unreadNotificationsCount ?? 0) > 0)
		                                        <span class="badge light text-white bg-warning rounded-circle">{{ $unreadNotificationsCount }}</span>
		                                    @endif
		                                </button>
									...
									<li class="nav-item dropdown  header-profile">
										<button class="nav-link btn btn-link border-0 p-0" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Kullanıcı menüsü">
												<img src="{{ $authUser->profile_image ? asset('storage/' . $authUser->profile_image) : asset('images/user.png') }}" width="56" alt=""/>
										</button>
										<div class="dropdown-menu dropdown-menu-end">
	                                    <div class="header-profile-info p-3 border-bottom mb-2">
	                                        <div class="d-flex align-items-center">
	                                             <img src="{{ $authUser->profile_image ? asset('storage/' . $authUser->profile_image) : asset('images/user.png') }}" width="50" height="50" class="rounded-circle me-3" alt=""/>
	                                             <div>
	                                                 <h6 class="mb-0">{{ $authUser->name }}</h6>
	                                                 <span class="fs-12 text-muted d-block">{{ $authUser->email }}</span>
	                                                 <div class="fs-12 text-primary font-w600">{{ $authUser->department->name ?? 'Departman Yok' }}</div>
	                                             </div>
	                                        </div>
	                                    </div>
